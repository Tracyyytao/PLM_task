            ...
            if dataset_path_str:
                ...
                for dataset_path in dataset_path_list:
                    ...
                    if not os.path.exists(dataset_path):
                        ...
                        lines = [json.loads(line.strip()) for line in
                                 open(dataset_path, 'r', encoding='utf8').readlines()]
                        df_dict = {
                            'text': lines[:5] + ['...']
                        }
                        df = pd.DataFrame.from_dict(df_dict)
...
def get_model():
    ...
    """
    with st.expander('', expanded=True):
        upload_model = st.checkbox('是否上传本地模型',
                                   help='上传本地模型文件，模型结构需和[这里](https://github.com/HarderThenHarder/transformers_tasks/blob/main/UIE/model.py)保持一致。')
...
def main():
    ...
    st.markdown(
        "[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&duration=600&pause=500&color=38F77FFF&vCenter=true&multiline=true&width=606&height=130&lines=import+torch;from+model+import+Model;Model.fit()_;Epoch+1+[=======================================];Get+Off+Work%EF%BC%81)](https://github.com/HarderThenHarder/transformers_tasks)")
    ...
    with da_page:
            ...
            with st.spinner('正例生成中，请稍等...'):
                if da_mode == '基于规则（Swap SPO）':
                    ...
                    if 'filling_model' not in st.session_state:
                        st.session_state['filling_model'] = T5ForConditionalGeneration.from_pretrained(
                            filling_model_path).to(device)
        ...
        if 'positive_samples' in st.session_state:
            ...
            if da_mode == '基于规则（Swap SPO）':
                ...
                with st.expander('同 Predicate 样本详情（若数据集较大建议先「下载生成结果」再展开，以防卡死）',
                                 expanded=False):
                    st.write('数据集下包含的所有Predicate(s)：')
                    st.code(f'{list(st.session_state["predicates_sentence_dict"].keys())}')
                    search_p = st.selectbox('查看数据集中指定的同P样本：',
                                            list(st.session_state["predicates_sentence_dict"].keys()))
