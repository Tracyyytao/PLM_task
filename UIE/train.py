...
parser.add_argument("--pretrained_model", default='../Huggingface/uie-base-zh', type=str,
...
parser.add_argument("--save_dir", default="./check", type=str, required=False,
...
parser.add_argument("--auto_da_epoch", default=0, type=int, required=False,
                    help="auto add positive/negative samples policy frequency.")
...
def auto_add_samples(
    ...
    with torch.no_grad():
        ...
        # * 添加正/负例
        negaitve_samples_generated_sample_num = int(
            len(generated_negative_samples) * args.auto_neg_rate)  # 随机采样等比例的新生成负例数据
        positive_samples_generated_sample_num = int(
            len(generated_positive_samples) * args.auto_pos_rate)  # 随机采样等比例的新生成正例数据
