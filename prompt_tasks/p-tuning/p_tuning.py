        ...
        for batch in train_dataloader:
            ...
            if args.rdrop_coef > 0:
                logits2 = model(input_ids=batch['input_ids'].to(args.device),
                                token_type_ids=batch['token_type_ids'].to(args.device)).logits
                ce_loss = (mlm_loss(logits, batch['mask_positions'].to(args.device), sub_labels, criterion, 1.0,
                                    args.device) + \
                           mlm_loss(logits, batch['mask_positions'].to(args.device), sub_labels, criterion, 1.0,
                                    args.device)) / 2
                kl_loss = rdrop_loss.compute_kl_loss(logits, logits2, device=args.device)
                loss = ce_loss + kl_loss * args.rdrop_coef
            else:
                loss = mlm_loss(logits, batch['mask_positions'].to(args.device), sub_labels, criterion, 1.0,
                                args.device)
